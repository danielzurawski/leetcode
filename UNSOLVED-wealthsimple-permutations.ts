...
const justMessing = (allocations: any[]): object[] => {
  const allPortfolios: any[][] = [];

  let currentPorfolio: any = {};
  
  allocations.forEach((allo: any) => {
    currentPorfolio = {...currentPorfolio, ...{[allo.symbol]: allo.allocation}};
    allocations.forEach((allo2: any) => {
      if (allo.symbol == allo2.symbol) return;
...
const permutationsRecurse = (
  ...
  for (let i = 0; i < tickers.length; ++i) {
    ...
    permutationsRecurse(
      tickers,
      {...usedTickers, ...{[ticker.secondary]: true}},
      {...currentPortfolio, ...singlePortfolio},
      // eslint-disable-next-line no-param-reassign
      allCombinations, 
      maxPortfolioLength,
    )
...
const permutations = (allocations: any[]): any[][] => {
